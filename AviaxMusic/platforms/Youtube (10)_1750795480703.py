...
def clean_title_for_api(title: str) -> str:
    """Clean YouTube title for better API search results"""
    if not title:
        return ""
    ...
    unwanted_patterns = [
        ...
        r'new.*?song',
        r'\d{4}',  # Remove years
        r'ft\.?',  # Remove "ft" or "ft."
        r'feat\.?',  # Remove "feat" or "feat."
        r'featuring',
        r'vs\.?',  # Remove "vs" or "vs."
        r'version',
        r'edition',
        r'remastered',
        r'deluxe',
    ...
    cleaned = re.sub(r'[^\w\s-]', ' ', cleaned)  # Keep only alphanumeric, spaces, and hyphens
    cleaned = re.sub(r'\s+', ' ', cleaned)  # Multiple spaces to single space
    cleaned = cleaned.strip()
    
    # If the cleaned title is too short, use original but clean it minimally
    if len(cleaned) < 3:
        # Just remove brackets and parentheses
        minimal_clean = re.sub(r'[\[\]()]', ' ', title)
        minimal_clean = re.sub(r'\s+', ' ', minimal_clean).strip()
        return minimal_clean if len(minimal_clean) > 2 else title
        ...
        if not video and not songvideo:
            try:
                ...
                if search_title:
                    ...
                    cleaned_title = clean_title_for_api(search_title)
                    logging.info(f"Searching Music API for: {cleaned_title}")
                    
                    # Get stream URL from our API
                    stream_url, api_title = await get_audio_stream_from_api(cleaned_title)
